...
with open(arquivo, "rt") as fasta, open("Python_08.codons-frame-1.nt", "w") as arquivo_saida:
	for line in fasta:
	...
	for identificador, sequencia in fastaDict.items():
		arquivo_saida.write(f"{identificador}-frame-1-codons\n")  #Escreve o cabeçalho formatado para a sequência
		codons = []  #Cria uma lista vazia para armazenar os códons da sequência atual

		#Iterando pela string da sequência, pulando de 3 em 3 bases (quadro de leitura 1)
...
print("Processamento concluído. Verifique o arquivo \"Python_08.codons-frame-1.nt\".")
